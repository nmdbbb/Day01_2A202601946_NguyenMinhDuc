...
def call_openai_mini(
    ...
    """
    return call_openai(model=OPENAI_MINI_MODEL,
                       prompt=prompt,
                       temperature=temperature,
                       top_p=top_p,
                       max_tokens=max_tokens)
...
def compare_models(prompt: str) -> dict:
    ...
    return {"gpt4o_answer":gpt4o_text,
...
def run_assistant(
    ...
    while True:
        ...
        turn_cost = estimate_cost(user_msg, reply_text)
        tokens_used += turn_cost["prompt_tokens"] + turn_cost["completion_tokens"]
